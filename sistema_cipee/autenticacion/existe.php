...
if ($_POST[documento]=='COPIA CERTIFICADA'){
        //valida que tenga los datos respectivos
	 if (($row['cedula']==$cedula) && ($row['titulo']!='') && ($row['periodo']!=''))
           {
               header("Location: ../consulta/ver_rep_copia_autenticacion.php?id=$cedula");
               }else{
               header("Location: ../actualizar/Mdatos.php?id=$row[cedula]&documento=$_POST[documento]");
            }
 ...
 if ($_POST[documento]=='CERTIFICACION NOTAS'){
        //valida que tenga los datos respectivos
	 if (($row['cedula']==$cedula) && ($row['titulo']!='') && ($row['periodo']!=''))
           {
               header("Location: ../consulta/ver_rep_certificacion.php?id=$cedula");
               }else{
               header("Location: ../actualizar/Mdatos.php?id=$row[cedula]&documento=$_POST[documento]");
            }
