...
$result4 = pg_query($con,"SELECT *
 FROM MATERIAL WHERE ESTADO='ACTIVO'
 order by descripcion
");
echo "<form id=form1 name=form1 method=post action=edit_inventario.php>";
...
echo "<td <small style=width: 50px font-weight: bold><b>COD</b></td>";
...
?>
<td <input autocomplete=OFF size="2" name="cod<? echo $c ?>" type="text"   value="<?echo $row4[5]?>" /> </td>
<?
echo "<td <small style=width: 50px >$row4[1]</td> ";
echo "<td <small style=width: 50px >$row4[2]</td> ";
echo "<td <small style=width: 50px >$row4[3]</td> ";
?>
<td <input autocomplete=OFF size="10" name="cant<? echo $c ?>" type="text"   value="<?echo $row4[3]?>" /> </td>
