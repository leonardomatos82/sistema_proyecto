...
$row = 1;
$handle = fopen("trabajadores.csv", "r"); //Coloca el nombre de tu archivo .csv que contiene los datos
...
while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) { //Lee toda una linea completa, e ingresa los datos en el array 'data'
 ...
 $num = count($data); //Cuenta cuantos campos contiene la linea (el array 'data')
    $row++;
    $cadena = "INSERT INTO trabajador(
            codigo, cedula, nombre, fecha_n, fecha_i, cargo, 
            departamento, sexo, tipo, sueldo, hijo, jerarquia, profesionalizacion, 
            antiguedad, hogar, chofer, moto, integral) Values ("; //Cambia los valores 'CampoX' por el nombre de tus campos de tu tabla y colócales los necesarios
$cont=1;    
