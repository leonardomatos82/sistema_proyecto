...
$row = 1;
$handle = fopen("BENEFICIARIO.csv", "r"); //Coloca el nombre de tu archivo .csv que contiene los datos
...
while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) { //Lee toda una linea completa, e ingresa los datos en el array 'data'
 ...
 $num = count($data); //Cuenta cuantos campos contiene la linea (el array 'data')
    $row++;
    $cadena = "INSERT INTO beneficiario (cod_credito,codigo_sec,id_organismo,expediente,tipo_persona,cuotas,
 cedula,rif,apellidos,nombres,nombre_empresa,periodo_cuotas,interes_mora,sobre_tasa,puntos_sobre_tasa,tasa_im,tasa_io,porcentaje_maq,
 pagos_judiciales,arrime_cosecha,direccion,telefono,ano_presupuestario,fecha_otorgamiento,fecha_regir,id_localidad,inicial,cap_financiable,
 intereses_ordinarios,monto_credito,id_credito,id_rubro,valor_tasa,p_cuota,monto_subsidio,interes_anual,cuotas_canceladas,saldo_actual,abono,
 cuotas_gracia,gracia_cancelada,id_programacion,id_tipo_pago,id_cuenta,subsidiado,irrecuperable,en_observacion,dpto_legal,observaciones ,
 control,status, usr) Values ("; //Cambia los valores 'CampoX' por el nombre de tus campos de tu tabla y colócales los necesarios
$cont=1;    
