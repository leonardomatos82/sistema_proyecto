...
var fichero_consulta = 'includes/days.php'; // DirecciÃ³n que devuelve los dÃ­as disponibles del mes y año
var dir_imagenes = 'includes/acc_calendar'; //Direcctorio donde se encuentren las imÃ¡genes
var preposiciones = {
...
var Meses = {
	ca : new Array('Gener','Febrer','MarÃ§','Abril','Maig','Juny','Juliol','Agost','Septembre','Octubre','Novembre','Desembre'),
...
var summary = {
	...
	es : 'Calendario con los dÃ­as disponibles correspondientes al mes de',
	...
	for (var i = 0; i < labels.length; i++) {
		...
		if(!document.getElementById('cal_' + asociado)){
			...
			if(elasociado.type == 'text' && elasociado.className.indexOf(label_class[lang.substr(0,2)]) != -1) {
				...
				enl_cal_img.setAttribute('src',dir_imagenes + '/acc_calendar.gif');
...
function calendario(cons_disp,todact,mes,year,nuevo,id,rango,dia) {
	...
	if(!year){
		//year = Hoy.getYear();//función Deprecated
