...
<?php
 ...
 if(strlen($nick) > $max_char && correcto) { echo "<script>Mensaje('El Nick es demasiado largo, Máximo $max_char caracteres')</script>"; $correcto=false; }
    ...
    for ($n = 0; $n < sizeof($nicks); $n++) 
	  {
		   if ($nicks[$n][0] == $nick)
		     if ($nicks[$n][1] != md5($pass))
		       {echo "<script>Mensaje('El Nick está registrado')</script>"; $correcto=false;} 	 
	  }
 ...
 // Corrección del problema de las urls, redirecciones,etc.
	...
	$timestamp = time();  
	 $timeout = $timestamp - 300;
	  ...
	  // Insertamos el mensaje en el archivo
        ...
        // Añadimos el nuevo mensaje a los antiguos 
