...
function registrarOperacion($con,$usuario,$descripcion,$ip) {
$hora=date("H:i:s");
  pg_query($con,"INSERT INTO transaccion (fecha,hora,operacion,login,ip) VALUES ('".date("Y-m-d")."', '$hora','".$descripcion."','".$usuario."','".$ip."')");
}
...
// funci�n para cambiar la fecha a formato Postgres  aaaa/mm/dd
	...
	return $lafecha; 
		...
		function cambiaf_a_postgres2($fecha){ 
	ereg( "([0-9]{2,2})-([0-9]{1,2})-([0-9]{1,4})", $fecha, $mifecha); 
...
// funci�n para cambiar la fecha a formato espa�ol  dd/mm/aaaa
	...
	return $lafecha; 
		...
		function cambiaf_a_dia($fecha){ 
	ereg( "([0-9]{2,4})-([0-9]{1,2})-([0-9]{1,2})", $fecha, $mifecha); 
	$lafecha=$mifecha[3]; 
	return $lafecha; 
	} 
	
		function cambiaf_a_mes($fecha){ 
	ereg( "([0-9]{2,4})-([0-9]{1,2})-([0-9]{1,2})", $fecha, $mifecha); 
	$lafecha=$mifecha[2]; 
	return $lafecha; 
	} 
	
		function cambiaf_a_anyo($fecha){ 
	ereg( "([0-9]{2,4})-([0-9]{1,2})-([0-9]{1,2})", $fecha, $mifecha); 
	$lafecha=$mifecha[1]; 
	return $lafecha; 
	} 
